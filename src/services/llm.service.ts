...
export class LLMService {
   ...
   */
  public async processTranscript(input: LLMProcessInput): Promise<VoiceProcessResponse> {
   ...
   */
  private async callGemini(input: LLMProcessInput, apiKey: string): Promise<VoiceProcessResponse | null> {
...
6. Return purely a valid JSON object strictly matching this JSON schema.`;
   ...
   */
  private async callOpenAI(input: LLMProcessInput, apiKey: string): Promise<VoiceProcessResponse | null> {
   ...
   */
  public localMultilingualExtractor(input: LLMProcessInput): VoiceProcessResponse {
    ...
    let candidateServices: CandidateServiceSummary[] = [];
    ...
    const missingFields: Array<"service" | "date" | "time" | "address"> = [];
    ...
    return {
      ...
      matchedService: fullMatchedService
        ? {
          ...
          priceType: fullMatchedService.priceType as any,
  ...
  private matchServiceFromCatalog(
    text: string,
    catalog: CatalogServiceItem[]
  ): { exactMatch?: CatalogServiceItem; candidates: CatalogServiceItem[] } {
    const lower = text.toLowerCase();
    ...
    const matchingCategoryServices: CatalogServiceItem[] = [];
    ...
    if (
      ...
      const repairServices = catalog.filter(
        (c) =>
   ...
   */
  private extractDate(text: string, tz: string = "Asia/Kolkata"): string | null {
    const lower = text.toLowerCase();
    const now = new Date();
   ...
   */
  private extractAddress(text: string, savedAddresses?: string[]): string | null {
   ...
   */
  private generateMultilingualResponse(ctx: {
    lang: string;
   ...
   */
  private formatAndValidateResponse(raw: any, input: LLMProcessInput): VoiceProcessResponse {
    ...
    let matchedServiceItem: CatalogServiceItem | null = null;
    if (serviceId) {
    ...
    const missingFields: Array<"service" | "date" | "time" | "address"> = [];
    if (!serviceId) missingFields.push("service");
    if (!raw.bookingDate) missingFields.push("date");
    if (!raw.bookingTime) missingFields.push("time");
    if (!raw.customerAddress) missingFields.push("address");
    ...
    return {
      ...
      matchedService: matchedServiceItem
        ? {
          ...
          priceType: matchedServiceItem.priceType as any,
