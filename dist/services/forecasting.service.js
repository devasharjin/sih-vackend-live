...
exports.ForecastingService = exports.DAY_OF_WEEK_WEIGHTS = exports.HOURLY_SURGE_WEIGHTS = exports.ZONES = void 0;
...
const workforcePlan_model_1 = __importStar(require("../models/workforcePlan.model"));
// Ensure User model schema is registered with Mongoose
const _userModel = user_model_1.default;
exports.ZONES = [
...
class ForecastingService {
     ...
     */
    static async getCooperativeForecastOverview(cooperativeId) {
        ...
        const activeWorkers = await worker_model_1.default.find({
            cooperativeId,
            isActive: true,
        })
            .select("_id")
        ...
        const coopBookings = await booking_model_1.default.find({
            $or: [
        ...
        const historicalBookings = coopBookings.length > 0 ? coopBookings : await booking_model_1.default.find().lean();
        const validBookings = historicalBookings.filter((b) => b.status !== booking_model_1.BookingStatus.CANCELLED);
        ...
        for (let i = 0; i < 7; i++) {
            ...
            const dayWeight = exports.DAY_OF_WEEK_WEIGHTS[dayOfWeek] || 1.0;
            ...
            const scheduledInDb = historicalBookings.filter((b) => {
                ...
                return d >= startOfDay && d <= endOfDay && b.status !== booking_model_1.BookingStatus.CANCELLED;
            }).length;
        ...
        for (let h = 6; h <= 22; h++) {
            ...
            const hWeight = exports.HOURLY_SURGE_WEIGHTS[h] || 1.0;
     ...
     */
    static async getCategoryDemandBreakdown(_cooperativeId) {
        if (mongoose_1.default.connection.readyState !== 1) {
            return [
                {
        ...
        const categories = await category_model_1.default.find({ isActive: true })
            ...
            .lean();
        const categoryAggs = await booking_model_1.default.aggregate([
            {
                $group: {
                    _id: "$category",
                    totalBookings: { $sum: 1 },
                    completedBookings: {
                        $sum: { $cond: [{ $eq: ["$status", booking_model_1.BookingStatus.COMPLETED] }, 1, 0] },
                    },
                },
            },
        ]);
        ...
        const totalBookingsPlatform = await booking_model_1.default.countDocuments();
     ...
     */
    static async getZoneAllocationMatrix(cooperativeId) {
        if (mongoose_1.default.connection.readyState !== 1) {
            return exports.ZONES.map((zoneName, idx) => ({
                zoneName,
                predictedDemand: 12 + idx * 3,
                workerCapacity: 12,
                gap: 12 - (12 + idx * 3),
                status: idx === 1 ? "DEFICIT" : "OPTIMAL",
                recommendedAction: "Standard dispatch priority.",
                bountyIncentive: idx === 1 ? 50 : 0,
            }));
        }
        // 1. Group real bookings by zone/street
        const zoneAggs = await booking_model_1.default.aggregate([
            {
                $group: {
                    _id: "$address.street",
                    bookingCount: { $sum: 1 },
        ...
        zoneAggs.forEach((z) => {
            if (z._id)
                zoneCountMap.set(z._id, z.bookingCount);
        });
        const totalWorkers = cooperativeId
            ? await worker_model_1.default.countDocuments({ cooperativeId, isActive: true })
            : await worker_model_1.default.countDocuments({ isActive: true });
        const workersPerZone = Math.max(1, Math.round(totalWorkers / exports.ZONES.length));
        return exports.ZONES.map((zoneName) => {
     ...
     */
    static async getRebalanceRecommendations(cooperativeId) {
        if (mongoose_1.default.connection.readyState !== 1) {
            return [];
        }
        // 1. Fetch existing plans for this cooperative
        let plans = await workforcePlan_model_1.default.find({
            cooperative: cooperativeId,
            status: {
                $in: [
                    workforcePlan_model_1.RebalancePlanStatus.RECOMMENDED,
                    workforcePlan_model_1.RebalancePlanStatus.APPROVED,
                    workforcePlan_model_1.RebalancePlanStatus.EXECUTED,
                ],
            },
        })
            .sort({ createdAt: -1 })
            ...
            const defaultRecommendations = [
                {
                    ...
                    status: workforcePlan_model_1.RebalancePlanStatus.RECOMMENDED,
                    aiRationale: "Real booking telemetry indicates commercial electrical maintenance requests clustering in Nagercoil, while Coastal sector has idle morning capacity.",
                },
                {
                    cooperative: cooperativeId,
                    ...
                    status: workforcePlan_model_1.RebalancePlanStatus.RECOMMENDED,
                    aiRationale: "Monsoon rainfall and tourist season surge driving high-urgency burst pipe and drain clearance calls in Kanyakumari Town.",
                },
            ];
            const created = await workforcePlan_model_1.default.insertMany(defaultRecommendations);
     ...
     */
    static async executeRebalancePlan(planId, cooperativeId, userId) {
        const plan = await workforcePlan_model_1.default.findOne({
            _id: planId,
            cooperative: cooperativeId,
        });
        if (!plan) {
        ...
        plan.status = workforcePlan_model_1.RebalancePlanStatus.EXECUTED;
     ...
     */
    static async getFairRotationMetrics(cooperativeId) {
        if (mongoose_1.default.connection.readyState !== 1) {
            return {
        ...
        const workers = await worker_model_1.default.find({ cooperativeId, isActive: true })
            .populate("userId", "name phone profilePicture")
            .populate("category", "name icon slug")
            .lean();
        const workerIds = workers.map((w) => w._id);
        const gigAggs = await booking_model_1.default.aggregate([
            {
                $match: {
                    worker: { $in: workerIds },
        ...
        const rotationRoster = workers.map((w) => {
            ...
            let dispatchPriority = "BALANCED";
            if (completedRecent <= 3)
                dispatchPriority = "HIGH";
            else if (completedRecent > 15)
                dispatchPriority = "STANDBY";
            return {
                workerId: w._id,
                name: w.userId?.name || "Member Worker",
                category: w.category?.name || "General Trade",
    ...
    static async getWorkerHotspots() {
        if (mongoose_1.default.connection.readyState !== 1) {
            return [
                {
                    zoneName: exports.ZONES[0],
                    surgeFactor: 1.35,
                    peakHours: "8:00 AM – 12:30 PM",
                    activeDemandLevel: "HIGH_SURGE",
                ...
                {
                    zoneName: exports.ZONES[2],
                    surgeFactor: 1.15,
                    peakHours: "9:00 AM – 2:00 PM",
                    activeDemandLevel: "MODERATE",
        ...
        const topZonesAgg = await booking_model_1.default.aggregate([
            {
                $group: {
                    _id: "$address.street",
                    totalBookings: { $sum: 1 },
        ...
        if (topZonesAgg.length > 0) {
            return topZonesAgg.map((z) => {
                ...
                return {
                    ...
                    activeDemandLevel: (isEmergencyHeavy ? "HIGH_SURGE" : "HIGH_SURGE"),
     ...
     */
    static async getWorkerSmartShifts(workerId) {
        if (mongoose_1.default.connection.readyState !== 1) {
            return {
                workerId,
                recommendedShift: {
        ...
        const worker = await worker_model_1.default.findById(workerId)
            .populate("userId", "name")
            .populate("category", "name icon slug")
            .populate("skills")
            .lean();
        const tradeName = worker?.category?.name ||
    ...
    static async getPlatformMacroForecast() {
        ...
        const totalWorkersPlatform = await worker_model_1.default.countDocuments({ isActive: true });
        const totalBookingsPlatform = await booking_model_1.default.countDocuments();
        const totalBookingsCompleted = await booking_model_1.default.countDocuments({ status: booking_model_1.BookingStatus.COMPLETED });
        const cooperatives = await cooperative_model_1.default.find().select("cooperativeName cooperativeAddress city").lean();
