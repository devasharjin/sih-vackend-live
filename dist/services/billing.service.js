...
exports.BillingService = exports.FIXED_TRANSPORT_FEE = void 0;
 ...
 */
exports.FIXED_TRANSPORT_FEE = 30;
class BillingService {
     ...
     */
    static calculateBillableHours(durationMinutes) {
     ...
     */
    static calculateWorkingDurationMinutes(startedAt, completedAt) {
        const start = new Date(startedAt);
        const end = new Date(completedAt);
        if (isNaN(start.getTime()) || isNaN(end.getTime())) {
            throw new appError_1.AppError("Invalid job start or completion timestamp", 400);
        }
        const diffMs = end.getTime() - start.getTime();
        if (diffMs < 0) {
            throw new appError_1.AppError("Job completion timestamp cannot be earlier than start timestamp", 400);
        }
     ...
     */
    static calculateServiceAmount(billableHours, firstHourRate, additionalHourRate) {
     ...
     */
    static calculateBillingAndDistribution(durationMinutes, pricing) {
        const { firstHourRate, additionalHourRate, cooperativePercentage, insurancePercentage, transportFee = exports.FIXED_TRANSPORT_FEE, } = pricing;
        if (firstHourRate < 0 || additionalHourRate < 0) {
            throw new appError_1.AppError("Service rates cannot be negative", 400);
        }
        if (cooperativePercentage < 0 || insurancePercentage < 0) {
            throw new appError_1.AppError("Share percentages cannot be negative", 400);
        }
        if (cooperativePercentage + insurancePercentage > 100) {
            throw new appError_1.AppError("Combined cooperative and insurance share cannot exceed 100%", 400);
        }
